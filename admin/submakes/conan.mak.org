...
ifndef ADMIN
    $(error Parent makefile must define 'ADMIN')
endif
ifndef TST
    $(error Parent makefile must define 'TST')
endif
ifndef VERSION_TRIPLET
    $(error Parent makefile must define 'VERSION_TRIPLET')
endif
ifndef BLD_DEBUG
    $(error cpp.mak must define 'BLD_DEBUG')
endif
ifndef BLD_PROD
    $(error cpp.mak must define 'BLD_PROD')
endif
...
_CONAN_INST_PROD        := $(BLD_PROD)/conan
_CONAN_PKG_PROD         := $(BLD_PROD)/package
_CONAN_PKG_PROD_VERIFY  := $(BLD_PROD)/package-verify
_CONAN_INST_DEBUG       := $(BLD_DEBUG)/conan
_CONAN_PKG_DEBUG        := $(BLD_DEBUG)/package
_CONAN_PKG_DEBUG_VERIFY := $(BLD_DEBUG)/package-verify

_CONAN_CFG_DIR := $(ADMIN)/conan
_CONAN_PY_FILE := $(_CONAN_CFG_DIR)/conanfile.py

# Sentinel files
_CONAN_INIT_DONE_DEBUG := $(_CONAN_INST_DEBUG)/.init-done
_CONAN_INIT_DONE_PROD  := $(_CONAN_INST_PROD)/.init-done
...
conan-prod: _ARG_INST_DIR := $(_CONAN_INST_PROD)
conan-prod: _ARG_LOCKFILE := $(_CONAN_LOCKFILE_PROD)
conan-prod: $(_CONAN_INIT_DONE_PROD)

conan-debug: _ARG_INST_DIR := $(_CONAN_INST_DEBUG)
conan-debug: _ARG_LOCKFILE := $(_CONAN_LOCKFILE_DEBUG)
conan-debug: $(_CONAN_INIT_DONE_DEBUG)

$(_CONAN_INIT_DONE_DEBUG) $(_CONAN_INIT_DONE_PROD):
	$(_CONAN_LOGIN_CHECK)
	@echo "(conan)  Installing libraries ..."
...
conan-lock-prod: _ARG_ADMIN_LOCKFILE := $(_CONAN_LOCKFILE_PROD)
conan-lock-prod: _ARG_INST_DIR       := $(_CONAN_INST_PROD)
conan-lock-prod: _ARG_BLD_TYPE       := Release
conan-lock-prod: _conan-lock1

conan-lock-debug: _ARG_ADMIN_LOCKFILE := $(_CONAN_LOCKFILE_DEBUG)
conan-lock-debug: _ARG_INST_DIR       := $(_CONAN_INST_DEBUG)
conan-lock-debug: _ARG_BLD_TYPE       := Debug
conan-lock-debug: _conan-lock2

_conan-lock1 _conan-lock2:
	@echo "Creating Conan lockfile: $(_ARG_ADMIN_LOCKFILE)"
	@mkdir -p $(_ARG_INST_DIR)
	$(CPP_BLD_CNTR_EXEC) conan lock create $(_CONAN_PY_FILE) \
	    --lockfile-out=$(_ARG_INST_DIR)/conan.lock \
	    -o sgn_build_type=$(_ARG_BLD_TYPE)
	cp $(_ARG_INST_DIR)/conan.lock $(_ARG_ADMIN_LOCKFILE)
...
_CONAN_PKG_VERIFY_SRC_DIR := $(TST)/packaging-verify
